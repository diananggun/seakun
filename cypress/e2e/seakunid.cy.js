...
describe('Testing Website Seakun id', () => {
    ...
    it('Test slide in carousel twitter' ,() => {
        ...
        cy.get('#testimony').should('be.visible')
        .scrollIntoView()
        .next()
    ...
    it('Test pengguna seakun youtube full' ,() => {
        cy.get('#container-pill').contains('Youtube').click()
        cy.wait(5000)
        cy.get('#pengguna').contains('full').should('be.visible')
        .then((el) => {
            if(Cypress.dom.isVisible(el)) {
                cy.get('#pengguna').find('button[data-v-9e7fa756]').contains('Pesan Sekarang').should('not.be.disabled')
            }else {
                cy.get('#pengguna').find('button[data-v-9e7fa756]').contains('Pesan Sekarang').should('be.disabled')
            }
        })
    })
    ...
    it('Make sure Sefitnes product button is disabled and button text is Segera Hadir' , ()=>{
        ...
        cy.get('@sefitnesProduct')
        .find('button')
        .should('be.disabled')
        .contains('Segera hadir')
    })
    ...
    // it('Should be able to close the "Pengaduan Layanan" floating button',()=>{
    //     cy.get('div.btn-float div.relative div[role="button"]').last().click()
